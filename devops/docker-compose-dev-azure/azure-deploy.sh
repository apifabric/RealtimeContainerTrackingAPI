...
resourcename="containertrackingsystem"  # lower case, only
resourcegroup="containertrackingsystem_rg"
...
# git clone https://github.com/apilogicserver/containertrackingsystem.git
# cd containertrackingsystem
...
az group create --name containertrackingsystem_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group containertrackingsystem_rg --sku S1 --is-linux
az container create --resource-group containertrackingsystem_rg --name containertrackingsystem --image apilogicserver/containertrackingsystem:latest --dns-name-label containertrackingsystem --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//containertrackingsystem.westus.azurecontainer.io:5656'
